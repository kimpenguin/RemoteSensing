...
#calculate export annual sum
r.series input=$rsTsList output=CHIRPS_sum_"$StartYear"_"$EndYear" method=sum
r.out.gdal input=CHIRPS_sum_"$StartYear"_"$EndYear"@"$mapset" output=$OutputDir/CHIRPS_sum_"$StartYear"_"$EndYear".tif
chmod 775 $OutputDir/CHIRPS_sum_"$StartYear"_"$EndYear".tif
